...
static bool calc_lft_vzw(struct inet6_ifaddr *ifp, u32 *minimum_lft)
{
	unsigned long expires;
	u32 route_lft;
	bool route_expires;

	route_expires = rt6_get_dflt_router_expires(ifp->idev->dev,
					       &expires);
	if (route_expires) {
		route_lft = (expires - ifp->tstamp) / HZ;
	...
	return route_expires;
}

static void calc_next_vzw(struct inet6_ifaddr *ifp, bool route_expires,
			  unsigned long *next, unsigned long age,
			  int is_expires, u32 minimum_lft)
{
	if (strncmp(ifp->idev->dev->name, "ccmni", 2) == 0) {
		if (is_expires || route_expires) {
...
restart:
		hlist_for_each_entry_rcu_bh(ifp, &inet6_addr_lst[i], addr_lst) {
			unsigned long age;
			u32 min_lft;
			bool route_expires = false;

			if (sysctl_optr == MTK_IPV6_VZW_ALL ||
			    sysctl_optr == MTK_IPV6_EX_RS_INTERVAL)
				route_expires = calc_lft_vzw(ifp, &min_lft);
			...
			if (ifp->valid_lft != INFINITY_LIFE_TIME &&
				...
				if (sysctl_optr == MTK_IPV6_VZW_ALL ||
					 ...
					 */
					calc_next_vzw(ifp, route_expires, &next, age,
						      0, min_lft);
				}
