...
static int32_t i2c_write_seq(struct aw9610x *aw9610x)
{
	...
	for (msg_idx = 0; msg_idx < addr_bytes; msg_idx++) {
		...
		//AWLOGI(aw9610x->dev, "w_buf_addr[%d] = 0x%02x", msg_idx, w_buf[msg_idx]);
	}
	msg_cnt = addr_bytes;
...
static int32_t i2c_read_seq(struct aw9610x *aw9610x, uint8_t *reg_data)
{
	...
	*/
	for (msg_idx = 0; msg_idx < addr_bytes; msg_idx++) {
		w_buf[msg_idx] = aw9610x->aw_i2c_package.init_addr[msg_idx];
		//AWLOGD(aw9610x->dev, "w_buf_addr[%d] = 0x%02x", msg_idx, w_buf[msg_idx]);
	}
	msg[0].addr = i2c->addr;
	msg[0].flags = AW9610X_I2C_WR;
	msg[0].len = msg_idx;
...
static void aw9610x_power_on_prox_detection(struct aw9610x *aw9610x)
{
	...
	while ((temp_time)--) {
		...
		if (reg_data == 1) {
			//AWLOGI(aw9610x->dev, "time = %d", temp_time);
						...
						struct aw_bin *aw_bin_data_s)
{
	...
	for (i = 0; i < aw_bin_data_s->header_info[0].valid_data_len;
		...
		//AWLOGI(aw9610x->dev, "reg_addr = 0x%04x, reg_data = 0x%08x", reg_addr, reg_data);
	}
...
static int32_t aw9610x_para_loaded(struct aw9610x *aw9610x)
{
	...
	for (i = 0; i < len; i = i + 2) {
		...
		//AWLOGI(aw9610x->dev, "reg_addr = 0x%04x, reg_data = 0x%08x", aw9610x_reg_default[i], aw9610x_reg_default[i+1]);
	}
...
static ssize_t sar_show_proxstatus_ch0(struct device *dev, struct device_attribute *attr, char *buf)
{
	uint8_t status;
	...
	AWLOGI(aw9610x->dev,"%s REG_STAT0 = 0x%08x\n", __func__, reg_value);
	status = (uint8_t)((reg_value & 0x3f000000) >> 24);
	if (((status >> 0) & 0x01) == 1)
	{
		state = 1;//near
		//pr_info("[CH0]Reading SAR state is:%d\n",state);
	}
	else
	{
		state = 2;//realse
...
static ssize_t sar_show_proxstatus_ch2(struct device *dev, struct device_attribute *attr, char *buf)
{
	uint8_t status;
	uint32_t reg_value = 0;
	int state = 0;
...
static ssize_t sar_show_proxstatus_ch4(struct device *dev, struct device_attribute *attr, char *buf)
{
	uint8_t status;
	uint32_t reg_value = 0;
	int state =0;
	...
	{
		state = 1;//near
		//pr_info("[CH4]Reading SAR state is:%d\n",state);
	}
	else
	{
		state = 2;//realse
...
static ssize_t sar_docalibration_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct aw9610x *aw9610x = dev_get_drvdata(dev);
...
static ssize_t aw9610x_aot_cali(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	ssize_t ret;
	uint32_t state;
	uint32_t data_en = 0;
	...
	ret = kstrtouint(buf, 10, &state);
	if (ret) {
		AWLOGE(aw9610x->dev, "%s: fail to set aot cali\n", __func__);
		aw9610x->cal_flag = 2;
		return ret;
	}
	...
	if (state != 0){
		...
		msleep(500);
		aw9610x->cal_flag = 1;
		AWLOGI(aw9610x->dev, "awsar calibration ok");
	}else{
		aw9610x->cal_flag = 2;
		AWLOGE(aw9610x->dev, "%s: fail to set aot cali\n", __func__);
	}
...
static ssize_t onoff_store(struct device *dev, struct device_attribute *attr,const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "0", 1))
	{
		...
		msleep(200);
		aw9610x->onoff_value = 0 ;
		AWLOGI(aw9610x->dev, "[AW96103]:onoff sar sensor is off \n");
	}
	if (!strncmp(buf, "1", 1))
	{
		aw9610x_i2c_write(aw9610x,0xF008,0x01);
		msleep(200);
		aw9610x->onoff_value = 1 ;
		AWLOGI(aw9610x->dev, "[AW96103]:onoff on \n");
	}
...
static ssize_t sar_info_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	uint32_t reg_value = 0;
	struct aw9610x *aw9610x = dev_get_drvdata(dev);
...
static DEVICE_ATTR(operation_mode, 0644, aw9610x_operation_mode_get, aw9610x_operation_mode_set);
...
static DEVICE_ATTR(sar_proxstatus_ch4,0444,sar_show_proxstatus_ch4,NULL);
static DEVICE_ATTR(sar_calibration, 0664, sar_docalibration_show,aw9610x_aot_cali);
...
static struct attribute *aw9610x_sar_attributes[] = {
	...
	&dev_attr_sar_proxstatus_ch4.attr,
...
static void aw9610x_irq_handle(struct aw9610x *aw9610x)
{
	...
	for (i = 0; i < AW_CHANNEL_MAX; i++) {
		...
		//AWLOGI(aw9610x->dev, "curr_state[%d] = 0x%x", j * AW_CHANNEL_MAX + i, aw9610x->aw_pad[j * AW_CHANNEL_MAX + i].curr_state);
			   ...
			   struct device_node *np)
{
	...
	val = of_property_read_u32_array(np, "channel_config", aw9610x->channel_config, ARRAY_SIZE(aw9610x->channel_config));
	if (val) {
		AWLOGI(aw9610x->dev, "aw9610x dts can not get channel config.\n");
	}
