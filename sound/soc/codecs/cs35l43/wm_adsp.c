...
int wm_adsp_load_coeff(struct wm_adsp *dsp)
{
	...
	if (file == NULL)
		return -ENOMEM;
	/*when 2nd firmware exist, this code will be open
	if (audio_boxid_get() == 1) {
		snprintf(file, PAGE_SIZE, "%s-%s-%s-2.bin", dsp->part, dsp->fwf_name,
			wm_adsp_fw[dsp->fw].file);
	} else*/
	...
	while (pos < firmware->size &&
		...
		default:
			adsp_err(dsp, "%s.%d: Unknown region type %x %d at %d\n",
				 file, blocks, type, type,pos);
			if(type == 14882)
			adsp_err(dsp, "%s.%d: cs35l43 music %x\n",
				 file, blocks, type);
			if(type == 25971)
			adsp_err(dsp, "%s.%d: cs35l43 ringtone %x\n",
				 file, blocks, type);
			if(type == 25970)
			adsp_err(dsp, "%s.%d: cs35l43 voice/voip %x\n",
				 file, blocks, type);
			break;
		}
...
static int wm_adsp_compr_attach(struct wm_adsp_compr *compr)
{
	...
	list_for_each_entry(tmp, &compr->dsp->buffer_list, list) {
		if (!tmp->name || !strcmp(strstr(compr->name, "dsp"),
					  strstr(tmp->name, "dsp"))) {
			buf = tmp;
			break;
		}
