...
//extern int snd_soc_new_compress(struct snd_soc_pcm_runtime *rtd, int num);
...
static SOC_VALUE_ENUM_SINGLE_DECL(cs35l43_dsprx4_enum,
				CS35L43_DSP1RX4_INPUT,
				0, CS35L43_INPUT_MASK,
				cs35l43_tx_input_texts,
				cs35l43_tx_input_values);

static const struct snd_kcontrol_new dsp_rx4_mux =
	SOC_DAPM_ENUM("DSPRX4 SRC", cs35l43_dsprx4_enum);

static SOC_VALUE_ENUM_SINGLE_DECL(cs35l43_dsprx5_enum,
				CS35L43_DSP1RX5_INPUT,
				0, CS35L43_INPUT_MASK,
				cs35l43_tx_input_texts,
				cs35l43_tx_input_values);

static const struct snd_kcontrol_new dsp_rx5_mux =
	SOC_DAPM_ENUM("DSPRX5 SRC", cs35l43_dsprx5_enum);

static SOC_VALUE_ENUM_SINGLE_DECL(cs35l43_dsprx6_enum,
				CS35L43_DSP1RX6_INPUT,
				0, CS35L43_INPUT_MASK,
				cs35l43_tx_input_texts,
				cs35l43_tx_input_values);

static const struct snd_kcontrol_new dsp_rx6_mux =
	SOC_DAPM_ENUM("DSPRX6 SRC", cs35l43_dsprx6_enum);
			   ...
			   struct snd_ctl_elem_value *ucontrol)
{
    ...
    dev_dbg(cs35l43->dev, "delta applying delta file %s\n",filename);
// -1 is music -2 is ringtone -3 is voip
		       ...
		       struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		cs35l43_hibernate(w, kcontrol, SND_SOC_DAPM_POST_PMD);
		break;
...
static const struct reg_sequence cs35l43_pdn_patch[] = {
	{ 0x0001510C, 0x00000010 },
	{ 0x00015110, 0x00000000 },
	{ 0x00000040, 0x00000055 },
	{ 0x00000040, 0x000000AA },
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		regmap_multi_reg_write(cs35l43->regmap, cs35l43_pup_patch, ARRAY_SIZE(cs35l43_pup_patch));

		//regmap_update_bits(cs35l43->regmap,CS35L43_BLOCK_ENABLES, 1, 1);
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		//regmap_update_bits(cs35l43->regmap, CS35L43_BLOCK_ENABLES,1,0);
		regmap_write(cs35l43->regmap, CS35L43_DSP_VIRTUAL1_MBOX_1,
				CS35L43_MBOX_CMD_AUDIO_PAUSE);
		regmap_multi_reg_write(cs35l43->regmap, cs35l43_pdn_patch, ARRAY_SIZE(cs35l43_pdn_patch));
...
static const struct snd_soc_dapm_widget cs35l43_dapm_widgets[] = {
	...
	SND_SOC_DAPM_MUX("DSP RX4 Source", SND_SOC_NOPM, 0, 0, &dsp_rx4_mux),
	SND_SOC_DAPM_MUX("DSP RX5 Source", SND_SOC_NOPM, 0, 0, &dsp_rx5_mux),
	SND_SOC_DAPM_MUX("DSP RX6 Source", SND_SOC_NOPM, 0, 0, &dsp_rx6_mux),
...
static const struct snd_soc_dapm_route cs35l43_audio_map[] = {
	...
	{"DSP RX3 Source", "Zero", "ASPRX1"},
	{"DSP1", NULL, "DSP RX3 Source"},

	{"DSP RX4 Source", "VMON", "VMON ADC"},
	{"DSP RX4 Source", "IMON", "IMON ADC"},
	{"DSP RX4 Source", "VPMON", "VPMON ADC"},
	{"DSP RX4 Source", "ASPRX1", "ASPRX1"},
	{"DSP RX4 Source", "ASPRX2", "ASPRX2"},
	{"DSP RX4 Source", "Zero", "ASPRX1"},
	{"DSP1", NULL, "DSP RX4 Source"},

	{"DSP RX5 Source", "VMON", "VMON ADC"},
	{"DSP RX5 Source", "IMON", "IMON ADC"},
	{"DSP RX5 Source", "VPMON", "VPMON ADC"},
	{"DSP RX5 Source", "ASPRX1", "ASPRX1"},
	{"DSP RX5 Source", "ASPRX2", "ASPRX2"},
	{"DSP RX5 Source", "Zero", "ASPRX1"},
	{"DSP1", NULL, "DSP RX5 Source"},

	{"DSP RX6 Source", "VMON", "VMON ADC"},
	{"DSP RX6 Source", "IMON", "IMON ADC"},
	{"DSP RX6 Source", "VPMON", "VPMON ADC"},
	{"DSP RX6 Source", "ASPRX1", "ASPRX1"},
	{"DSP RX6 Source", "ASPRX2", "ASPRX2"},
	{"DSP RX6 Source", "Zero", "ASPRX1"},
	{"DSP1", NULL, "DSP RX6 Source"},	
	...
	{"DSP RX4 Source", "VMON FS2", "VMON ADC"},
	{"DSP RX5 Source", "VMON FS2", "VMON ADC"},
	{"DSP RX6 Source", "VMON FS2", "VMON ADC"},
	...
	{"DSP RX4 Source", "IMON FS2", "IMON ADC"},
	{"DSP RX5 Source", "IMON FS2", "IMON ADC"},
	{"DSP RX6 Source", "IMON FS2", "IMON ADC"},
	...
	{"DSP RX4 Source", "VBSTMON", "VBSTMON ADC"},
	{"DSP RX5 Source", "VBSTMON", "VBSTMON ADC"},
	{"DSP RX6 Source", "VBSTMON", "VBSTMON ADC"},
...
static int cs35l43_set_pdata(struct cs35l43_private *cs35l43)
{
		...
		regmap_update_bits(cs35l43->regmap, CS35L43_BST_IPK_CTL,
				CS35L43_BST_IPK_MASK, cs35l43->pdata.bst_ipk<< CS35L43_BST_IPK_SHIFT);
...
static const struct reg_sequence cs35l43_dsp_init_ext_standby[] = {
	{ 0x0001510C, 0x00000010 },
	{ 0x00015110, 0x00000000 },
	{ 0x00000040, 0x00000055 },
	{ 0x00000040, 0x000000AA },
...
static int cs35l43_dsp_init(struct cs35l43_private *cs35l43)
{
	...
	regmap_multi_reg_write(cs35l43->regmap, cs35l43_dsp_init_ext_standby,
				ARRAY_SIZE(cs35l43_dsp_init_ext_standby));
...
static int cs35l43_compr_open(struct snd_compr_stream *stream)
{
       ...
       if (!strcmp(rtd->codec_dai->name, "cs35l43-dsp-textlog"))
...
static const struct snd_compr_ops cs35l43_compr_ops = {
